...
#define _BSD_SOURCE // for usleep() in unistd.h
...
#include <unistd.h> // sleep(), man 3 sleep for details
...
int main() {
    ...
    if (pthread_mutex_init(&g_mutex_tid_arr2, NULL) != 0) {
    ...
    pthread_t *tid_arr3;
    tid_arr3 = (pthread_t *) malloc(g_N_tid_arr3 * sizeof(pthread_t));

    // allocate memory to args for thread functions
    int *arg_tid_arr3;
    arg_tid_arr3 = (int *) malloc(g_N_tid_arr3 * sizeof(int));
